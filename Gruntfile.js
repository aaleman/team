    ...
    grunt.initConfig({
        ...
        concat: {
            ...
            build: {
                ...
                dest: 'build/<%= meta.version %>/team-<%= meta.version %>.js'
            }
        },
        uglify: {
            options: {
            ...
            build: {
                ...
                dest: 'build/<%= meta.version %>/team-<%= meta.version %>.min.js'
            }
        },
        copy: {
            build: {
                files: [
                    {   expand: true, cwd: './src', src: ['pa-config.js'], dest: 'build/<%= meta.version %>/' },
        ...
        htmlbuild: {
            build: {
                ...
                options: {
                    beautify: true,
                    scripts: {
                        'js': 'build/<%= meta.version %>/team-<%= meta.version %>.min.js',
                        'vendor': [
...
//                            'build/<%= meta.version %>/vendor/jquery-ui-1.10.3*/css/**/jquery-ui*min.css'
        ...
        rename: {
            html: {
                files: [
                    {src: ['build/<%= meta.version %>/team.html'], dest: 'build/<%= meta.version %>/index.html'}
                ]
            }
        },
        hub: {
    ...
    grunt.registerTask('log-deploy', 'Deploy path info', function () {
        grunt.log.writeln("DEPLOY COMMAND: scp -r build/" + grunt.config.data.meta.version + " cafetero@mem16:/httpd/bioinfo/www-apps/panels/");
    });
